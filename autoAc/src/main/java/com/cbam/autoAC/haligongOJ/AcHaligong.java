...
public class AcHaligong {
    ...
    public Boolean init(HashMap map) {
        ...
        String sql = "select problem_id from autoac where acmer='" + this.userName + "'";
        ResultSet rs = dbUtil.select(sql);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String nowDate = sdf.format(new Date());
        String sqlLimit = "select count(88) from autoac where acmer='" + this.userName + "' and date_format(create_time, '%Y-%m-%d') = '" + nowDate + "'";
        ResultSet rsLimt = dbUtil.select(sqlLimit);
        try {
            while(rsLimt.next()) {
                int num = rsLimt.getInt(1);
                if(num > 30) {
                    System.out.println("初始化失败！当天使用次数已达上限！（30）");
                    return false;
                }
            }
            while (rs.next()) {
